...
import PhotoEngine from './photo-engine';
...
export default class EmojiKey extends Component {
    ...
    preloadEmojiElements() {
        ...
        Emoji.map((category) => {
            ...
            category.items.map((emoji, index) => {
                emojiHtml += `<button id="U${PhotoEngine.hashHex(emoji.code)}" class="pbemoji-sprite pbemoji-${category.key} pbemoji-${index}" title="${emoji.name}"></button>`;
            });
    ...
    getEmojiInfo(code) {
        for(let e=0; e<Emoji.length; e++) {
            ...
            let index = category.items.map(function(e) { return PhotoEngine.hashHex(e.code); }).indexOf(code);
            if (index > -1) {
